...
export const UpdateQuiz = () => {
  ...
  // const quizid1= location.search ? new URLSearchParams(props.location.search).get('data'):null;
  ...
  useEffect(() => {
    async function getCategories() {
      try {
        ...
        const response1 = await axios.get(`/quiz/${data}`);
        // console.log(response1.data.title);
        settitle(response1.data.title);
        setdescription(response1.data.description);
        setmaxMarks(response1.data.maxMarks)
        setcategory(response1.data.category.cid)
        setnumberofQuestions(response1.data.numberofQuestions);
        setstatus(true)
        
        // console.log(response1.data.active);
      } catch (error) {
      ...
      <form className='max-w-xl mx-auto p-4 text-xl font-bold bg-white border text-gray-700 dark:text-white' onSubmit={submit}  >
        ...
        <div className='mt-4 w-3/4  mx-autow-3/4  mx-auto'>
          <label className="relative inline-flex items-center cursor-pointer ">
            <input type="checkbox" checked={status} onClick={() => { setstatus(!status) }} className="sr-only peer" />
        ...
        <div className=' mt-10 flex items-center justify-center'>
          <button type="submit" className="text-white bg-primary-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Update quiz</button>
        </div>
      </form>
    </section>
  )
